...
export const useJobSeeker = () => {
	...
	const { data } = useQuery({
		queryKey: ['jobSeekers'],
		queryFn: async () => jobSeekersService.getJobSeekers(),
		enabled: typeof window !== "undefined",
	});

	const getJobSeekersProfile = useQuery({
		queryKey: ['jobSeekerProfile'],
		queryFn: async () => jobSeekersService.getJobSeekerProfile(),
		enabled: typeof window !== "undefined",
	});
	const JobSeekersProfile = getJobSeekersProfile.data;

	return { data, JobSeekersProfile, useJobSeekersUpdateJobSeeker, useJobSeekersCreateJobSeekerPosting };
};
