import { CheckCircle, Eye, FileText, MapPin } from 'lucide-react';
			...
			<div className='flex gap-3 w-[100%]'>
				<button
					onClick={() => {
						...
						if (role) {
							router.push(`/${role}/jobs/${job.id}`);
						} else {
							router.push(`/jobs/${job.id}`);
						}
					}}
					className='flex-1 bg-gray-100 text-gray-700 py-3 px-4 rounded-lg font-medium hover:bg-gray-200 transition-colors cursor-pointer'
				>
					Details
				</button>
			</div>
